...
if __name__ == '__main__':
    ...
    # [注意] MSELoss(reduction="none") 返回逐元素 loss，依赖 train_battery_seq 中 .mean() 归约
    #   若后续改为 reduction="mean" 可直接去掉 .mean()，减少一次归约操作
    ...
    for epoch in tqdm_iter:
        ...
        # [严重 Bug] type 变量未定义！运行时会抛出 NameError
        #   应改为 "lstm"（与 init_state 中的 type 参数对应）
        ...
        if val_loss < val_loss_best:
            ...
            stop_count = 0
            torch.save(net.state_dict(), 'time_seq_best.opt')  # [风格] .opt 扩展名不常规，建议用 .pt 或 .pth
        else:
            stop_count += 1
    ...
    # [缺失] 训练全程只记录了一个混合 loss，无法判断 SOC 和分类各自学得怎样
    #   建议 train/val 各自记录 loss_soc、loss_cls、cls_acc，分任务画曲线
